...
void KnowledgeBase::runKnowledgeBase() {
    ...
    while (ros::ok()) {
        ...
        // update TILs
        std::vector<rosplan_knowledge_msgs::KnowledgeItem>::iterator tit =
...
bool KnowledgeBase::queryKnowledge(
    rosplan_knowledge_msgs::KnowledgeQueryService::Request &req,
...
bool KnowledgeBase::updateKnowledgeConstraintsOneOf(
    rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Request &req,
    rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Response &res) {
...
bool KnowledgeBase::updateKnowledge(
    rosplan_knowledge_msgs::KnowledgeUpdateService::Request &req,
    rosplan_knowledge_msgs::KnowledgeUpdateService::Response &res) {
    ...
    case rosplan_knowledge_msgs::KnowledgeUpdateService::Request::
        REMOVE_KNOWLEDGE:
...
// bool
// KnowledgeBase::updateKnowledgeArray(rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Request
// &req, rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Response &res) {
bool KnowledgeBase::updateKnowledgeArray(
    ros::ServiceEvent<
        rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Request,
        rosplan_knowledge_msgs::KnowledgeUpdateServiceArray::Response> &event) {
...
void KnowledgeBase::removeFact(
    const rosplan_knowledge_msgs::KnowledgeItem &msg) {
    // remove domain attribute (predicate) from knowledge base
    std::vector<rosplan_knowledge_msgs::KnowledgeItem>::iterator pit;
    for (pit = model_facts.begin(); pit != model_facts.end();) {
        if (KnowledgeComparitor::containsKnowledge(msg, *pit)) {
            ROS_INFO("KCL: (%s) Removing Fact (%s,%i)",
                     ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
                     msg.is_negative);
            pit = model_facts.erase(pit);
        } else {
...
void KnowledgeBase::addKnowledge(rosplan_knowledge_msgs::KnowledgeItem &msg) {
    ...
    case rosplan_knowledge_msgs::KnowledgeItem::FACT: {
        ...
        for (pit = model_facts.begin(); pit != model_facts.end(); pit++) {
            if (KnowledgeComparitor::containsKnowledge(msg, *pit)) {
                ROS_WARN("KCL: (%s) fact (%s%s) already exists",
                         ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
                         param_str.c_str());
                return;
            }
            ...
            if (KnowledgeComparitor::containsKnowledge(msg, *pit)) {
                ROS_INFO("KCL: (%s) Setting fact (%s%s) is_negative=%i",
                         ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
                         param_str.c_str(), (1 - msg.is_negative));
        ...
        // add fact
        ROS_INFO("KCL: (%s) Adding fact (%s%s, %i)",
                 ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
                 param_str.c_str(), msg.is_negative);
    ...
    case rosplan_knowledge_msgs::KnowledgeItem::FUNCTION: {
        ...
        if (!function_exists &&
            msg.assign_op == rosplan_knowledge_msgs::KnowledgeItem::AP_ASSIGN) {
...
void KnowledgeBase::addMissionGoal(rosplan_knowledge_msgs::KnowledgeItem &msg) {
    ...
    for (pit = model_goals.begin(); pit != model_goals.end(); pit++) {
        if (KnowledgeComparitor::containsKnowledge(msg, *pit)) {
            ROS_WARN("KCL: (%s) Goal (%s%s) already posted",
                     ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
                     param_str.c_str());
            return;
        }
    ...
    // add goal
    ROS_INFO("KCL: (%s) Adding mission goal (%s%s)",
             ros::this_node::getName().c_str(), msg.attribute_name.c_str(),
             param_str.c_str());
...
bool KnowledgeBase::getInstances(
    rosplan_knowledge_msgs::GetInstanceService::Request &req,
    rosplan_knowledge_msgs::GetInstanceService::Response &res) {
...
bool KnowledgeBase::getPropositions(
    rosplan_knowledge_msgs::GetAttributeService::Request &req,
...
bool KnowledgeBase::getMetric(
    rosplan_knowledge_msgs::GetMetricService::Request &req,
    rosplan_knowledge_msgs::GetMetricService::Response &res) {
    res.metric = model_metric;
    return true;
}

bool KnowledgeBase::setSensedPredicate(
    rosplan_knowledge_msgs::SetNamedBool::Request &req,
    rosplan_knowledge_msgs::SetNamedBool::Response &res) {
...
KnowledgeBase::KnowledgeBase(ros::NodeHandle &n) {
    ...
    // status publishers
    status_pub =
        _nh.advertise<rosplan_knowledge_msgs::StatusUpdate>("status/update", 100);
