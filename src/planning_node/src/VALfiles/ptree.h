...
namespace VAL1_2 {
    ...
    enum constraint_sort {
        E_ATEND, E_ALWAYS, E_SOMETIME, E_WITHIN, E_ATMOSTONCE, E_SOMETIMEAFTER,
...
/*-----------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    void pc_list<pc>::display(int ind) const {
        for (typename pc_list<pc>::const_iterator i = _Base::begin(); i != _Base::end(); ++i) ELT(*i);
    };
...
// - hence we don't make it a pc_list.
    ...
    public:
        ...
        virtual void display(int ind) const {
            ...
            //for (typed_symbol_list::iterator i= begin(); i!=end(); ++i)
            for (const_iterator i = _Base::begin(); i != _Base::end(); ++i) ELT(*i);
        };
...
/*----------------------------------------------------------------------------
  ...
  --------------------------------------------------------------------------*/
    ...
    public:
        ...
        virtual ~uminus_expression() { delete arg1; };
  ...
  ---------------------------------------------------------------------------*/
    ...
    public:
        ...
        constraint_goal(constraint_sort c, goal *req, goal *tri) : cons(c),
                                                                   requirement(req), trigger(tri), deadline(0),
                                                                   from(0) {};
    ...
    public:
        ...
        virtual ~simple_goal() { delete prop; };
    ...
    class comparison
            : public goal, public binary_expression // proposition? // No, function inequality, eg. (> a b) --mc
    {
    private:
        comparison_op op;
 ...
 *-------------------------------------------------------------------------*/
    ...
    private:
        ...
        bool body_changed;
    public:
        derivation_rule(proposition *p, goal *g, var_symbol_table *v) : vtab(v), head(p), body(g),
                                                                        body_changed(false) {};
