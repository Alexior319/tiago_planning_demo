...
namespace KCL_rosplan {
	 ...
	 */
	bool KnowledgeComparitor::inequalityTrue(const xyz_knowledge_msgs::KnowledgeItem &a, const std::vector<xyz_knowledge_msgs::KnowledgeItem> &modelFunctions) {
		...
		switch(a.ineq.comparison_type) {
			case xyz_knowledge_msgs::DomainInequality::GREATER:   return lhs>rhs;
			case xyz_knowledge_msgs::DomainInequality::GREATEREQ: return lhs>=rhs;
			case xyz_knowledge_msgs::DomainInequality::LESS:      return lhs<rhs;
			case xyz_knowledge_msgs::DomainInequality::LESSEQ:    return lhs<=rhs;
			case xyz_knowledge_msgs::DomainInequality::EQUALS:    return lhs==rhs;
		}
	 ...
	 */
	double KnowledgeComparitor::evaluateExpression(const xyz_knowledge_msgs::ExprComposite &expr, const std::vector<xyz_knowledge_msgs::KnowledgeItem> &modelFunctions) {

		std::vector<xyz_knowledge_msgs::ExprBase> tokens = expr.tokens;

		bool pending_operand = false;
		std::vector<xyz_knowledge_msgs::ExprBase> operator_stack;
		std::vector<xyz_knowledge_msgs::ExprBase> operand_stack;
		for(int i=0;i<tokens.size();i++) {
			xyz_knowledge_msgs::ExprBase token = tokens[i];
			switch(token.expr_type) {
				case xyz_knowledge_msgs::ExprBase::OPERATOR:
					operator_stack.push_back(token);
					pending_operand = false;
					break;
				case xyz_knowledge_msgs::ExprBase::CONSTANT:
				case xyz_knowledge_msgs::ExprBase::FUNCTION:
				case xyz_knowledge_msgs::ExprBase::SPECIAL:

					if(pending_operand) {
						while (operand_stack.size()>0) {
							xyz_knowledge_msgs::ExprBase lhs = token;
							xyz_knowledge_msgs::ExprBase rhs = operand_stack.back();
							operand_stack.pop_back();
							xyz_knowledge_msgs::ExprBase op = operator_stack.back();
							operator_stack.back();
							double value = KnowledgeComparitor::evaluateOperation(
									...
									op);
							token.expr_type = xyz_knowledge_msgs::ExprBase::CONSTANT;
		...
		xyz_knowledge_msgs::ExprBase result = operand_stack.back();
		operand_stack.pop_back();
	...
	double KnowledgeComparitor::evaluateOperation(const double &lhs, const double &rhs, const xyz_knowledge_msgs::ExprBase &op) {
		...
		switch(op.op) {
			case xyz_knowledge_msgs::ExprBase::ADD: return lhs+rhs;
			case xyz_knowledge_msgs::ExprBase::SUB: return lhs-rhs;
			case xyz_knowledge_msgs::ExprBase::MUL: return lhs*rhs;
			case xyz_knowledge_msgs::ExprBase::DIV: return lhs/rhs;
			case xyz_knowledge_msgs::ExprBase::UMINUS: return -lhs;
		}
	 ...
	 */
	double KnowledgeComparitor::getValue(const xyz_knowledge_msgs::ExprBase &expr, const std::vector<xyz_knowledge_msgs::KnowledgeItem> &modelFunctions) {
		switch(expr.expr_type) {
			case xyz_knowledge_msgs::ExprBase::CONSTANT:
				return expr.constant;
			case xyz_knowledge_msgs::ExprBase::FUNCTION:
				std::vector<xyz_knowledge_msgs::KnowledgeItem>::const_iterator fit = modelFunctions.begin();
	 ...
	 */
	bool KnowledgeComparitor::containsInstance(const xyz_knowledge_msgs::KnowledgeItem &a, std::string &name) {
