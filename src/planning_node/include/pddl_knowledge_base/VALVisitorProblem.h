...
#ifndef ROS_PROJECT_VALVISITORPROBLEM_H
#define ROS_PROJECT_VALVISITORPROBLEM_H
...
namespace planning_node {
    ...
    public:
        ...
        xyz_knowledge_msgs::DomainFormula last_func_term;

        std::map<std::string, std::vector<std::string> > instances;
        std::map<std::string, std::vector<std::string> > constants;
        std::vector<xyz_knowledge_msgs::KnowledgeItem> facts;
        std::vector<xyz_knowledge_msgs::KnowledgeItem> functions;
        std::vector<xyz_knowledge_msgs::KnowledgeItem> goals;
        xyz_knowledge_msgs::KnowledgeItem metric;

        /* timed initial literals */
        std::vector<xyz_knowledge_msgs::KnowledgeItem> timed_initial_literals;

        /* return methods */
        std::map<std::string, std::vector<std::string> > returnInstances();

        std::vector<xyz_knowledge_msgs::KnowledgeItem> returnFacts();

        std::vector<xyz_knowledge_msgs::KnowledgeItem> returnFunctions();

        std::vector<xyz_knowledge_msgs::KnowledgeItem> returnGoals();

        xyz_knowledge_msgs::KnowledgeItem returnMetric();

        std::vector<xyz_knowledge_msgs::KnowledgeItem> returnTimedKnowledge();
...
#endif //ROS_PROJECT_VALVISITORPROBLEM_H
